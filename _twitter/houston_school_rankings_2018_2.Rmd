...
Among independent school districts, rural districts occupy the highest and lowest graduation rates.
...
They also show a clear positive correlation with the Children At Risk's grade ranking.
...
p_grad_heat_map <- h %>%
  ...
  ungroup() %>%
  mutate(grad_range = cut_interval(grad_rate, 7),
         grad_range2 = cut(grad_rate, breaks = c(0.5, 0.55, 0.6, 0.65, 0.75, 0.8, 0.85, 0.9, 1.0))) %>%
  arrange(grad_range2) %>%
