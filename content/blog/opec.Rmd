...
post_cut <- post_cut %>%
  ...
  mutate(group = if_else(group == TRUE, "OPEC, Russia,\nand others", "United States"))
...
post_cut_overview <- post_cut %>%
  ...
  labs(x = "Date", y = paste0("Thousand Barrels Per Day"),
       title = paste0("OPEC's and Russia's loss is the US's gain"),
...
In September 2016, OPEC announced it's first output cut in 8 years. Three months later, Russia, Mexico, Kazakhstan, and Oman announced they would join OPEC by cutting their own output.
...
That addition by the United States is `r sprintf("%.0f", prod_diff / 1000)` million bpd more than the change in production among OPEC, Russia, Mexico, Kazakhstan, and Oman - combined.
