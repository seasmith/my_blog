...
```{r import_oil_price, eval=FALSE}
read_chunk("us_rig_counts_ii.Rmd", labels = "import_oil_price")
```
...
```{r read_csv_rig_count, include=FALSE, purl=TRUE}
...
```{r wrangle_data_rig_counts, include=FALSE, purl=TRUE}
basin <- basin %>%
...
# ____ Create text table for `geom_text()`
...
# ____ Merge datasets with sf data
...
# ____ BAR - Change from week ago; rig count
...
# ____ BAR - Rig count; 52 week percent change
...
# ____ BAR - Total current rig total
rc_total <- this_week %>%
  arrange(value) %>%
