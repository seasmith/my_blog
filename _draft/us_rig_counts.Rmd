...
key_dates <- rc_basin %>%
  ...
  arrange(desc(Date)) %>%
  pull(Date) %>%
  magrittr::extract(c(this_week      = 1,
                      week_ago       = 2,
                      month_ago      = 5,
                      six_months_ago = 27,
                      year_ago       = 53)) %>%
...
c(this_week,
  ...
  month_ago,
  six_months_ago,
  year_ago) %<-% map(nms, ~filter(total_comparison, Date == key_dates[[.x]]))

# Find most recent week's oil and gas type
type_comparison <- rc_basin %>%
  group_by(basin, key) %>%
  arrange(Date) %>%
  mutate(chng_1wk = (value - lag(value)) / lag(value),
         chng_4wk = (value - lag(value, 4)) / lag(value, 4),
