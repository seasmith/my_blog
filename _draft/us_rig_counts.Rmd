...
load("text_mods2.RData")
load("line_barnett.RData")
load("line_cana_woodford.RData")
load("line_barnett2.RData")
load("line_cana_woodford2.RData")
...
# ggplot() +
#     geom_point(aes(x, y, color = ele), us_48_only_no_ocean_gray_raster, size = 0.4) +
...
map_layout <- ggplot() +
  ...
  geom_sf(data = states_48, fill = "#00000000",
          color = nord_palettes$polarnight[4L]) +
  geom_text(data = text_mods2, aes(X, Y, label = Shale_play),
            size = 3.5, family = "Open Sans", color = "gray90") +
  geom_line(data = line_barnett2, aes(X, Y), color = "gray80") +
  geom_line(data = line_cana_woodford2, aes(X, Y), color = "gray80") +
...
# map_layout <- ggplot() +
#   geom_point(aes(x = x, y = y, color = ele), us48) +
#   geom_sf(data = states_48, fill = "#00000000",
#           color = nord_palettes$polarnight[4L]) +
...
#     scale_color_gradient(low = nord_palettes$polarnight[1L], high = nord_palettes$polarnight[4L]) +
#   # scale_x_continuous(expand = expand_scale()) +
#   # scale_y_continuous(expand = expand_scale()) +
#   theme_nord(16) +
#   theme(axis.title = element_blank()) +
...
rc_map <- map_layout  +
  ...
  coord_sf(datum = NA, crs = st_crs(4326)) +
...
rc_map_grob$grobs[[which(rc_map_grob$layout$name == "caption")]] <- lgnd

rc_grobs <- arrangeGrob(rc_map_grob,
