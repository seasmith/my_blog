...
rc_total_chart <- rc_total %>%
  ...
  labs(title = paste0("Which basins are drillers targetting?"),
       subtitle = paste0("Week of: ", key_dates[["this_week"]]),
       x = NULL,
       y = paste0("Rig Count"),
...
```{r plot_rig_type_draw, fig.height=9.2}
...
bc_oil_percent_1wk <- oil_dat %>%
  ggplot() +
  geom_col(aes(reorder(basin, percent_1wk, identity),
               percent_1wk,
               fill = factor(sign(percent_1wk)))) +
  scale_y_continuous(minor_breaks = NULL, labels = blg_frmt_percent()) +
  scale_fill_manual(values = oil_wk1_colors) +
  guides(fill = FALSE) +
  coord_flip()
...
bc_gas_percent_1wk <- bc_oil_percent_1wk %+%
  gas_dat +
  scale_fill_manual(values = gas_wk1_colors)
```
...
bc_oil_change_1wk <- oil_dat %>%
  ggplot() +
  geom_col(aes(reorder(basin, change_1wk, identity),
               change_1wk,
               fill = factor(sign(change_1wk)))) +
  scale_y_continuous(minor_breaks = NULL) +
  scale_fill_manual(values = oil_wk1_colors) +
  guides(fill = FALSE) +
  coord_flip()
