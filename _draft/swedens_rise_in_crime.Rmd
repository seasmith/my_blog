...
In the speech, Trump commented about "last night in Sweden", which prompted UKIP politician Nigel Farage two days later to state that Malmo, Sweden is the "rape capital" of Europe. Farage implied this was due to Sweden's migrant population, particularly refugees.
...
plot_swe1 <- swe %>%
  ...
  labs(caption = " ") +
  theme_classic(base_size = 20, base_family = "Lato") +
...
plot_swe2 <- swe %>%
  ...
  labs(caption = "Source: Brå (Reported Offenses); UN Refugee Agency (UNHCR)") +
  theme_classic(base_size = 20, base_family = "Lato") +
  theme(strip.background = element_blank(),
        strip.text = element_text(size = 22),
        panel.spacing.x = unit(1, "cm"), 
...
ggsave(plot_swe1, file = f_swe1 <- "~/R/my_blog/_draft/sweden/plot_swe1.png", height = 15, width = 7, dpi = 600)
ggsave(plot_swe2, file = f_swe2 <- "~/R/my_blog/_draft/sweden/plot_swe2.png", height = 15, width = 7, dpi = 600)
```
...
img_swe1 <- img_swe1 %>%
  image_join(
    image_blank(4200, 250, "white") %>%
      image_annotate("Crime Rates Per 100,000 Persons (1950-2017)",
                     size = 185, color = "black", font = "Lato",
...
img_swe2 <- img_swe2 %>%
  image_join(
    image_blank(4200, 250, "white") %>%
      image_annotate("Crime Rates And Asylum Decisions (2000-2017)",
                     size = 185, color = "black", font = "Lato",
                     location = geometry_point(50, 5))
    , .
  ) %>%
...
ggsave(plot_not_rape, file = f_not_rape <- "~/R/my_blog/_draft/sweden/plot_not_rape.png", height = 7, width = 7, dpi = 600)
...
f_not_rape %>%
  image_read() %>%
  {
    ...
    image_blank(i$width, 230, "white") %>%
      ...
      # image_append(stack = TRUE) %>%
      image_join(
        image_blank(i$width, 200, "white") %>%
          image_annotate("Sexual Crime Rates (Per 100,000 Persons)",
                         size = 130, font = "Lato", color = "black",
                         location = geometry_point(70, 5))
      ) %>%
      image_append(stack = TRUE) %>%
      image_join(img)
