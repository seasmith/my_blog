...
```{r purl_docs, include=FALSE, eval=FALSE}
knitr::opts_chunk$set(purl = TRUE)
f_name <- tools::file_path_sans_ext

p_fils <- c("us_state_prod.Rmd",
            "us_oil_price.Rmd")

td <- tempdir()

f_state_prod <- file.path(td, "us_state_prod.R")
f_oil_price <- file.path(td, "us_oil_price.R")

knitr::knit("us_state_prod.Rmd", tangle = TRUE)
knitr::knit("us_oil_price.Rmd", tangle = TRUE)
```

```{r read_chunk_docs, include=FALSE, eval=FALSE}
# knitr::read_chunk(f_state_prod)
# knitr::read_chunk(f_oil_price)
```

<!-- Source docs and get data -->

```{r source_docs, include=FALSE, eval=FALSE}
source(f_state_prod, TRUE)
source(f_oil_price, TRUE)
```
...
state_prod %>%
  ...
  geom_col(fill = blg::blg_colors$green) +
  coord_flip()
  ...
  img <- it_loc %>%
    map2_chr(it_num, ~{
      gg <- ggplot(filter(dat, location == .x), aes(total, price)) +
        geom_smooth(method = "lm", color = blg_colors$blue, se = FALSE) +
