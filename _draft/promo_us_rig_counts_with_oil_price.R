...
(rt_line <- rc_og %>%
  ...
  geom_text(aes(x, y + 60, label = y), tibble(x = as.Date("1989-02-01"), y = c(500, 1000, 1500)),
            color = "gray90", family = "Open Sans", size = 3) +
  scale_color_viridis_c(option = "D", breaks = pretty_breaks(2)) +
  scale_x_date(expand = expand_scale(),
               minor_breaks = NULL) +
  scale_y_continuous(expand = expand_scale(),
                     minor_breaks = NULL) +
...
(
  i_plot <- i_plot %>%
    ...
    image_annotate("Oil prices have risen from $50 in October 2017 to $65 in",
                   location = geometry_point(3, 36),
                   color = clr, font = fnt, size = 19) %>%
    image_annotate("January 2018 - a 30% increase.",
